...
public class VideoBo {
	...
	public ResponseApi createSubtitle(String audioUrl, String language) {
		...
		try {
			...
			if (responseCode == HttpURLConnection.HTTP_OK) {
				try (InputStream input = connection.getInputStream();FileOutputStream output = new FileOutputStream("E:/Downloads/uploads/output.zip")) {
					byte[] buffer = new byte[4096];
                    int bytesRead;
                    while ((bytesRead = input.read(buffer)) != -1) {
                        output.write(buffer, 0, bytesRead);
                    ...
                    String[] extractedFiles = unzip("E:/Downloads/uploads/output.zip","E:/Downloads/uploads");
	...
	public String setSubclip(String srtUrl, String originalUrl) {
        ...
        String outputVideoPath = convertPath("E:/Downloads/uploads/" + "result" + getUUID() + ".mp4");
